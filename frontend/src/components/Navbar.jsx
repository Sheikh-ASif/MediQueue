...
const Navbar = () => {
  ...
  return (
    <nav className="backdrop-blur-md bg-white/60 border-b border-white/30 shadow-md fixed top-0 w-full z-30 flex items-center justify-between px-6 py-3">
      ...
      <h1
        onClick={() => navigate("/")}
        className="text-2xl font-extrabold text-[#4f46e5] cursor-pointer"
      >
        🏨 MediQueue
      </h1>
      ...
      <div className="flex items-center gap-4 relative">
        ...
        {token && userData ? (
          <div className="relative">
            <div
              onClick={() => setShowDropdown(!showDropdown)} // ✅ click instead of hover
            ...
            {/* ✅ DROPDOWN (works on mobile too) */}
            {showDropdown && (
              <div className="absolute right-0 top-12 w-56 bg-white shadow-lg rounded-lg">
                <p
                  onClick={() => {
                    navigate("/my-profile");
                ...
                <p
                  onClick={logout}
                  className="p-3 cursor-pointer text-red-500 hover:bg-gray-100"
                >
                  Logout
                </p>
              </div>
          ...
          <button
            ...
            className="hidden md:block bg-blue-500 text-white px-5 py-2 rounded-full"
          >
            Create Account
          </button>
        )}
      ...
      {/* ✅ MOBILE MENU */}
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform transition ${
