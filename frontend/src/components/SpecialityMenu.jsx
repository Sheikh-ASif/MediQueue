...
const SpecialityMenu = () => {
  return (
    ...
    >
      ...
      {/* Speciality Cards */}
      <div className="flex sm:justify-center gap-6 pt-8 w-full overflow-x-auto scrollbar-hide snap-x snap-mandatory">
        {specialityData.map((item, index) => (
          <Link
            onClick={() => scrollTo(0, 0)}
            key={index}
            to={`/docters/${item.speciality}`}
            className="flex flex-col items-center bg-white shadow-md rounded-2xl px-6 py-4 cursor-pointer flex-shrink-0 hover:shadow-lg hover:-translate-y-2 transition-all duration-300 snap-center"
          >
            <img
